...
var events = [
    ...
    "{name}1 says they'll never leave.",
    ...
    "{name}1 shoots a flaming arrow to start a flamewar before settling in to watch the chaos unfold.", 
...
var deaths = [
    ...
    '@death{name}1 is caught uwuing by {name}2, who dislikes them for it.', 
    ...
    "After being unfollowed by @death{name}1, {name}2 roasts them over boiling heat with 'Nice unfollow. You thought I wouldnt notice, did you? I had some genuine hopes for your future but you threw all that out of the window… Blocked + ratio + don’t care + generic newgen'.",
    ...
    '@death{name}1 was ratioed by {name}2.',
    ...
    '{name}1 makes a controversial bait post. @death{name}2 failed to see their true intentions, getting themself into a flamewar.',
...
document.getElementById('menu-button').onclick = () => { window.location = window.location; };
...
function SaveAmount(participants) {
    ...
    let div = document.getElementsByClassName('contestant-template')[0];
...
function Start() {
    let divs = document.getElementsByClassName('contestant');
    for (var i in divs) {
...
function proceed() {
    ...
    if (document.getElementById("name-confirmation").style.display = 'block') {
        document.getElementById("name-confirmation").style.display = "none";
    }
    if (dayActive) {
        if (names.length == 1) {
...
async function Generate() {
    ...
    while (tbd.length != 0) {
        ...
        let count = GetNameCount(event);

        let people = [];
        
        for (let i = 0; i < count; i++) {
            let person = tbd[Math.floor(Math.random()*tbd.length)];
...
function GetNameCount(event) {
    var ptrn = /{name}([0-9]+)/g;
    var matches = event.match(ptrn);
    var count = 1;
    if (matches != null && matches.length > 1) {
        for (let i = 0; i < matches.length; i++) {
          let num = matches[i].slice(-1);
...
function randomEvent() {
    let deathChance = window.sessionStorage.getItem("deathChance");
